jQuery(function(jQuery) {
    ...
    awl_light_image_gallery = {
        ...
        init: function() {
			 ...
			 */
            jQuery('#add-new-photer').on('click', function(event) {
                ...
                file_frame.on('select', function() {
                    ...
                    for (var i = 0; i < length; i++) {
                        awl_light_image_gallery.get_thumbnail(images[i]['id']);
                    }
                });
                file_frame.open();
            });
			...
			jQuery('#remove-all-photos').on('click', function() {
                if (confirm('Are sure to delete all images?')) {
                    awl_light_image_gallery.ul.empty();
                }
                return false;
            });
        ...
        get_thumbnail: function(id, cb) {
            ...
            jQuery.post(ajaxurl, data, function(response) {
                awl_light_image_gallery.ul.append(response);
                cb();
            });
        }
    };
    awl_light_image_gallery.init();
});